...
//        'multisig' => 'Multisignature',
    ...
    public function setStatusNotification($val)
    {
        $this -> status_notification = $val;
    }
    ...
    public function isDisputedOrAppealed()
    {
	    $isDisputed = false;
	    
        if(!empty($this->dispute_id))
        {
	        $isDisputed = $this->dispute->state != 'closed';
        }
    ...
    public function canMakeDispute() : bool
    {
        ...
        if($this -> isBuyer()) return true; //Only Byer can init dispute || $this -> isVendor()
	...
	{
		if(!$this->dispute->isAllowedToEscalate())
		{
			throw new RequestException('You cannot Escalate dispute!');
		}
		
		if($this->dispute->isEscalated())
		{
			throw new RequestException('You cannot Escalate dispute again!');
		}
		
		try {
			
			DB::beginTransaction();
				$dispute = Dispute::find($this->dispute_id);
				$dispute->state = Dispute::ESCALATED_STATE;
				$dispute->escalated_by_user_id = auth()->id();
				$dispute->save();
			DB::commit();
			
			event(new ProductDisputeEscalated($this, auth()->user()));
			
		} catch (\Exception $e) {
			DB::rollBack();
			throw new RequestException('Escalation dispute Failed! Please try again!' . $e -> getMessage());
		}
	...
	{
		if(!$this->dispute->isAllowedToAppeal())
		{
			throw new RequestException('Dispute has been Appealed!');
		}
		...
		try {
			...
			DB::beginTransaction();
				...
				$dispute = Dispute::find($this->dispute_id);
				$dispute->state = Dispute::APPEAL_STATE;
				$dispute->appeal_by_user_id = auth()->id();
				$dispute->appeals_count = $dispute->appeals_count + 1;
				$dispute->save();
				
				$appeal = new \App\Appeals();
				$appeal->purchase_id = $this->id;
				$appeal->appealer_id = $dispute->appeal_by_user_id;
				$appeal->save();
			...
			DB::rollBack();
			throw new RequestException('Appeal dispute Failed! Please try again!' . $e -> getMessage());
		}
