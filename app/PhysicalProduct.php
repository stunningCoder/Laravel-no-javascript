...
class PhysicalProduct extends User
{
    ...
    public function getShippingFromToDetails()
    {
    	$shipping_from = '<b>Shipping from:</b> ' . $this->shipsFrom();
    	
    	
    	$countries = ' All';
    	
    	$shipping_to = '<b>Shipping to:</b> ';
    	
    	if($this->countries_option == 'include')
	    {
		    $countries = '<i class="fas fa-globe-europe" title ="' . $this->countriesLong() . '"></i>';
	    }
    	else if($this->countries_option != 'all')
	    {
		    $countries = $this->countries_option  . ' <i class="fas fa-globe-europe" title ="' . $this->countriesLong() . '"></i>';
	    }
  
	
	
	    return '<p class="shippping_container">' . $shipping_from . '<br>' . $shipping_to . $countries. '</p>';
    }
