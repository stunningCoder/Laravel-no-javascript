...
trait Purchasable {
    ...
    {
    	if( $this -> offer -> product->active == false)
		    throw new RequestException('Product is not available anymore.');
    ...
    public function delivered()
    {
        ...
        // state must be 'sent' to be delivered
        throw_unless($this->state=='sent', new RequestException('This purchase is already sent!'));
        ...
        try{
            ...
            // logout the exception message
            $message = "Purchase $this->id :" . $e ->getMessage();
    ...
    public function makeDispute($message)
    {
        if(!$this->canMakeDispute())
            throw new RequestException('You don\'t have permission to make dispute!');

        try{
            DB::beginTransaction();
            // Make dispute
            $newDispute = new Dispute();
            $newDispute->disputed_by_user_id = auth()->id();
            $newDispute -> save();
            // Make message
    ...
    public function resolveDispute(string $winnerId)
    {
        ...
        if(!$this -> isBuyer($winner) && !$this -> isVendor($winner))
            throw new RequestException('Winner must be vendor or buyer!');
        ...
        try{
            ...
            // Set the winner
            $this->dispute->state = 'closed';
            $this->dispute->resolved_by_user_id = auth()->user()->id;
     ...
     * @throws RequestException
     */
    public function resolveAppeal(string $winnerId)
    {
        $winner = User::find($winnerId);
        if(is_null($winner)) throw new RequestException('This user can not be winner!');
        ...
        try{
	        ...
	        try {
		        ...
		        $appeal = \App\Appeals::where('purchase_id', $this->id);
		        $appeal->update([
			        'resolver_id' => auth()->user()->id
		        ]);
		
	        } catch (\Exception $e) {
		        throw new RequestException('Something went wrong with Appeal, please try again!' . $e -> getMessage());
	        }
	        
	        $previous_winner_id =  $this->dispute->winner_id;
            // Set the winner
            $this->dispute->winner_id = $winner->id;
            $this->dispute->resolved_by_user_id = auth()->user()->id;
            
         
	        if (empty($previous_winner_id)) {
		        $this->getPayment()->resolved(['receiving_address' => $winner->coinAddress($this->getPayment()->coinLabel())->address]);
	        }
