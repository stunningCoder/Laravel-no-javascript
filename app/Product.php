...
class Product extends Model
{
    ...
    public function toSearchableArray() {
        ...
        $product = $this->with(['category' ,'shipping'])->with('user')
	        ->where('id', $this->id)
	        ->where('active', 1)
	        ->first();
...
//        $array['mesure'] = $product->mesure;
        ...
        if ($this->isPhysical()){
	        ...
	        $countries = $physicalProduct->countriesArray();
	
	        $all_countries = array_keys(config('countries'));
	        
            if($physicalProduct->countries_option == 'include')
            {
	            $array['to_country_code'] = $countries;
            }
            else if($physicalProduct->countries_option == 'exclude')
            {
	            $array['to_country_code'] = implode(',', array_diff($all_countries, $countries));
	            
            } else if($physicalProduct->countries_option == 'all') { //ALL countries
	            $array['to_country_code'] =implode(',', $all_countries);
            }
            
            //all default not needed
    ...
    public function getTypeAttribute() {
        return $this->isPhysical() ? 'physical' : ($this->isDigital() ?'digital':'');
    }
    ...
    /**
     * Collection of shipping
     *
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function shipping() {
        return $this->hasMany(\App\Shipping::class, 'product_id');
    }
    
    public function isFreeDelivery() {
    	return $this->shipping()->where('price', '=', 0)->exists() ? '<span class="free_delivery">Free Shipping</span>' : '';
    }
