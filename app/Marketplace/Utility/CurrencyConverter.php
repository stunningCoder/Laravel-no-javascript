...
class CurrencyConverter
{
    ...
    public static function getSymbol($localValue = 'USD'): string {
        ...
        $user_local_currency = session()->get('local_currency');
        if($user_local_currency == 'AUD' ||
            $user_local_currency == "NZD" ||
            $user_local_currency == "CAD" ||
            $user_local_currency == "HKD" ||
            $user_local_currency == "SGD" ||
            $user_local_currency == "MXN"
        )
        {
            return '$';
        }
        //dd(Intl::getCurrencyBundle()->getCurrencySymbol($user_local_currency));
        if(!empty($user_local_currency)){
            return Intl::getCurrencyBundle()->getCurrencySymbol($user_local_currency);
        }else{
