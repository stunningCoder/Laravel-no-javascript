...
class Cart
{
    ...
    public function addToCart(Product $product, int $quantity, string $coin, ?Shipping $shipping = null, string $message = null, $type = 'normal')
    {
    	if($product->active == false)
		    throw new RequestException('Product is not available!');
        // validate shipping
    ...
    public function items()
    {
	    ...
	    //validate items are active == 1
	    if (!empty($cartItems)) {
		    foreach ($cartItems as $id => $item) {
			    if (!Product::where('id', $id)->where('active', 1)->exists()) {
				
				    session()->flash('error', 'Product "' . $item->offer->product->name . '" was remove because its not available anymore!');
				    $this->removeFromCart($item->offer->product);
			    }
		    }
	    }
    ...
    public function removeFromCart(Product $product)
    {
        ...
        Cartdb::remove($cartItems[$product -> id]);
