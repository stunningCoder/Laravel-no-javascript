...
class User extends Authenticatable
{
    ...
    public static $permissionsLong = [
        ...
        'logs'           => 'Logs',
        'disputes'       => 'Disputes',
        'disputeappeals'       => 'Dispute Appeals',
        'tickets'        => 'Tickets',
        'deletetickets'        => 'Remove Tickets',
        'vendorpurchase' => 'Vendor Purchases',
        'purchases'      => 'Purchases',
    ];

    public static $vendorPermissions = [
        'productexport',
        'productimport'
    ];

    public static $vendorPermissionsLong = [
     ...
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function products($inactive = false)
    {
    	if(!$inactive)
	    {
    ...
    /**
     * Return limit of recent products
     *
     * @param int $limit
     *
     * @return \Illuminate\Database\Eloquent\Collection|\Illuminate\Support\Collection
     */
    public function recentProducts($limit = 3)
    {
        return $this->products()->limit($limit)->get();
    }
    ...
    {
	    $per_page = 12;
    	//$per_page =  config('marketplace.products_per_page');
    	
	    return \App\Product::where('active', true)
	...
	public function hasProductCryptoCoins(Product $product, $buyCoin)
	{
		
		$product_coins = $product->getCoins();
		...
		$noProductCtypto = count(array_intersect($product_coins, $userCoins)) == 0;
		
		if($noProductCtypto) {
			return -1;
		}
		
		if(!$noProductCtypto && !in_array($buyCoin, $userCoins)) {
			return  1;
		}
    ...
    public function hasPermissions()
    {
        ...
        //Exclude Vendor specific permissions
        return $this->permissions()->exists() && (bool) $this->permissions()->whereNotIn('name', self::$vendorPermissions)->count();
    }
    ...
    public function setPermissions(array $permissions)
    {
        ...
        if (!empty(array_diff($permissions, self::$permissions)) && !empty(array_diff(array_diff($permissions, self::$permissions), self::$vendorPermissions))) {
    ...
    public function getPgpPublicStatus(){
        return $this->pgp_public_status;
    }
