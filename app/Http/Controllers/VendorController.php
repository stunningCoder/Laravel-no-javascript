...
class VendorController extends Controller
{
    ...
    public function vendor()
    {
        return view('profile.vendor',
            [
                'myProducts' => auth() -> user() -> products(true) -> paginate(config('marketplace.products_per_page')),
    ...
    public function addBasicShow($type = 'physical')
    {
        ...
        $is_physical = ($type == 'physical') ? 1 : 0;

        return view('profile.vendor.addbasic', [
            'product_type' => $type,
	        'categories'       => Category::roots($is_physical),
	        'categoryState'    => CategoryFilter::defaultState($is_physical),
    ...
    {
    	if(empty( session('product_offers')))
	    {
		    session()->flash('errormessage', 'Please add Offers!');
    ...
    public function addDigitalShow()
    {
    	...
    	if(empty(session('product_offers')))
	    {
		    session() -> flash('errormessage', 'Product must have at least 1 Offer!');
		    session() -> flash('error', 'You don\'t have any offer please add at least one!');
		    return redirect() -> back();
	    }
    ...
    public function addImage(NewImageRequest $request, Product $product = null)
    {
        ...
        try{
            ...
            session() -> flash('errormessage', $e -> getMessage() . ' [' . __LINE__ . ']');
        }
    ...
    {
        $myProduct = auth() -> user() -> products(true) -> where('id', $id) -> first();
	    ...
	    $is_physical = ($myProduct->type == 'physical') ? 1 : 0;
    ...
    public function sale(Purchase $sale)
    {
	    ...
	    session()->forget('return_cofirm_sale_url');
    ...
    public function confirmSent(Purchase $sale)
    {
	    ...
	    {
	    	session()->put('return_cofirm_sale_url', route('profile.sales.sent.confirm', [ 'sale' => $sale ]));
		    $unlock_messages_url = route('profile.sales.messages.decrypt.show.sale');
	    }
    	else {
		    session()->forget('return_cofirm_sale_url');
    	}
        ...
        return view('profile.purchases.confirmsent', [
            ...
            'sale' => $sale,
            'unlock_messages_url' => $unlock_messages_url
        ]);
    }
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function markAsSentWithMessage(Purchase $sale, NewMessageRequest $request)
    {
        ...
        try{
	        ...
	        if (session()->has('private_rsa_key_decrypted') && !empty($_POST['message'])) {
		
		        $sender = $sale->vendor->user;
		        $receiver = $sale->buyer;
		        $conversation = Conversation::findWithUsersOrCreate($sender, $receiver);
		
		        $this->authorize('update', $conversation);
		        $request->persist($conversation);// Persist the request
		        session()->forget('return_cofirm_sale_url');
		        session() -> flash('success', 'You have successfully marked sale as sent and sent message to buyer!');
	        }
	        else {
		        session() -> flash('success', 'You have successfully marked sale as sent!');
	        }
