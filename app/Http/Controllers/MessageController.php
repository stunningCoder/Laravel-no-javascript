...
class MessageController extends ProfileController
{
    ...
    public function decryptKeyPost(DecryptMessagesRequest $request) {
	    ...
	    if(session()->has('return_cofirm_sale_url'))
	    {
	    	$return_cofirm_sale_url = session()->get('return_cofirm_sale_url');
		    session()->forget('return_cofirm_sale_url');
		    return redirect($return_cofirm_sale_url);
	    }
    ...
    /**
     * Shows page that requests password to decrypt rsa key for Vendor to add message to sent sale
     */
    public function decryptKeyShowSale(Request $request) {
	    return view('profile.messagekeysale');
    }
    
    /**
     * Shows page that requests password to decrypt rsa key for Vendor to add message to sent sale
     */
    public function decryptKeyPostSale(DecryptMessagesRequest $request) {
