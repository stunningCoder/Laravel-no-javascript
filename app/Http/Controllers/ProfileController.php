...
class ProfileController extends Controller
{
    ...
    public function change2fa($turn)
    {
        try{
            ...
            session() -> flash('success', 'You have changed your 2FA settings.');
        }
        catch (RequestException $e){
            session() -> flash('errormessage', $e -> getMessage());
        }
    ...
    {
    	$userCoins = $product->user->hasProductCryptoCoins($product, $request->coin);
    	
    	if($userCoins == 0) {
		    session() -> flash('errormessage', "You don\'t have any payment addresses!");
		    return redirect()->back();
		    
	    } else if($userCoins == -1) {
		    session() -> flash('errormessage', "You don\'t have the same payment address as Product!");
		    return redirect()->back();
		    
	    } else if($userCoins == 1) {
		    session() -> flash('errormessage', "Please, select payment address that you have!");
		    return redirect()->back();
    ...
    {
    	if($purchase->isBuyer()) //Buyer can't cancel purchase
	    {
	    	return redirect() -> back();
	    }
	...
	/**
	 * Returns view for confirming close dispute
	 *
	 * @param Purchase $purchase
	 * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View\
	 */
	public function confirmCloseDispute(Purchase $purchase)
	{
		if(!$purchase->isBuyer()) //Buyer can't cancel purchase
		{
			session() -> flash('error', 'Only Buyer can close this dispute!');
			return redirect() -> back();
		}
		
		return view('profile.purchases.confirmclosedispute', [
			'purchase' => $purchase
		]);
	}
	 ...
	 * @return \Illuminate\Http\RedirectResponse
	 */
	public function closeDispute( Purchase $purchase)
	{
		
		if($purchase->buyer->id != auth()->user()->id )
		{
			session() -> flash('error', 'Buyer can close this dispute!');
			return redirect() -> route('profile.purchases.single', $purchase);
		}
		
		try{
			
			$purchase -> resolveDispute($purchase->vendor->id);
			session() -> flash('success', 'You have successfully closed this dispute!');
		}
		catch (RequestException $e){
			$e -> flashError();
		}
...
//
	...
	/**
	 * Solve/UnSolve ticket request by User
	 *
	 * @param Ticket $ticket
	 * @return \Illuminate\Http\RedirectResponse
	 */
    ...
    public function announcements(){
        ...
        return view('profile.announcements',['data' => $data]);
    }
    
    public function ReportItemtickets(string $user,string $itemId){
        $ticket = null;
        // Tickets
        if(!is_null($ticket)){
            $replies = $ticket -> replies() -> orderByDesc('created_at') -> paginate( config('marketplace.products_per_page'));
        ...
        return view('profile.tickets', [
            ...
            'ticket' => $ticket,
            'reportedToUser' => $user,
            'reportedByUser' => auth()->user()->username,
            'reportedItem' => $itemId
        ]);
    }

    public function pgpStatusChange(Request $request,bool $status){

        $user = Auth::user();
        if($user != null){
            $pgp = User::where('id',$user->id)->update([
                'pgp_public_status' => $status
            ]);
            return redirect()->back();
        }else{
            abort(404);
    ...
    public function showPGPKey(Request $request ,string $user_id){
        if(!empty($user_id)){
            $data = User::where('id',$user_id)->first();
            if($data->pgp_public_status == true){
                echo $data->pgp_key;
            }else{
                return "No key Found!";
            }
