...
@section('title','Product Status Change')
...
@section('content')
    ...
    <div class="modal fade in show position-static d-block" tabindex="-1" role="dialog">
        <form action="{{route('profile.vendor.product.status.post',$product)}}" method="post">
            {{csrf_field()}}
            <div class="modal-dialog" role="document">
                <div class="modal-content" style="background-color: #1414147a;">
                    <div class="modal-header">
                        <h5 class="modal-title">Change product status to {{$product->active == 0 ? 'Published' : 'Inactive'}}</h5>

                    </div>
                    <div class="modal-body">
                        <p>Confirm changing status of product "{{ $product->name }}" to {{$product->active == 0 ? 'Published' : 'Inactive'}}</p>
             ...
             {{--           <h4>Important Notice:</h4>
                    ...
                    <div class="modal-footer text-center justify-content-center">
                        <button type="submit" class="btn btn-success">Confirm to {{$product->active == 0 ? 'Publish' : 'Inactive'}}</button>
