...
<hr>
<form method="POST" action="{{ route('profile.vendor.product.delivery.new', (!empty($physicalProduct-> product)) ? $physicalProduct->product : null) }}">
    {{ csrf_field() }}
    <div class="form-row">

        <div class="col-md-4 my-2">
{{--            IF YOU EDIT value of options, you must edit it app/Http/Requests/Product/NewShippingRequest.php   on line 33 --}}
            <select name="name" id="name" class="form-control">
                <option selected disabled>Delivery Type</option>
                 <option value="Priority (Tracked)" {{ old('name') == 'Priority (Tracked)' ? 'selected' : '' }}>Priority (Tracked)</option>
                 <option value="Standard (Not Tracked)" {{ old('name') == 'Standard (Not Tracked)' ? 'selected' : '' }}>Standard (Not Tracked)</option>
                 <option value="Free Shipping (Tracked)" {{ old('name') == 'Free Shipping (Tracked)' ? 'selected' : '' }}>Free Shipping (Tracked)</option>
                 <option value="Free Shipping (Not Tracked)" {{ old('name') == 'Free Shipping (Not Tracked)' ? 'selected' : '' }}>Free Shipping (Not Tracked)</option>
            </select>
            @error('name', $errors)
        ...
        <div class="col-md-4 my-2">
            <input type="text" maxlength="30" class="form-control @error('duration', $errors) is-invalid @enderror" name="duration" value="{{ old('duration') }}" placeholder="Duration (i.e. 1 week)">
        ...
        <div class="col-md-4 my-2">
            ...
            <select class="form-control" name="countries_option" id="countries_option">
                @foreach(\App\PhysicalProduct::$countriesOptions as $short => $optionName)
                    <option value="{{ $short }}" @if( !empty( $physicalProduct -> countries_option) && $short == $physicalProduct -> countries_option) selected @endif>{{ $optionName }}</option>
                @endforeach
            </select>
