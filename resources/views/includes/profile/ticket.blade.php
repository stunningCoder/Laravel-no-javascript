<h3 class="mb-2" style="word-wrap: break-word;">Ticket: {{ $ticket -> title }}</h3>
<hr>
@if(!$ticket -> solved)
<form method="POST" action="{{ route('profile.tickets.message.new', $ticket) }}">
    ...
    <div class="form-group text-right">
            ...
            @if($ticket->user_id == auth()->id())
                <a href="{{ route('profile.ticket.solve', $ticket) }}" class="btn btn-warning">
                    Solve Ticket
                </a>
            @else
...
@else
    <div class="alert text-center alert-success">
        ...
        @if($ticket->user_id == auth()->id())
            <a href="{{ route('profile.ticket.solve', $ticket) }}" class="btn btn-outline-danger btn-sm">Unsolve</a>
        @else
