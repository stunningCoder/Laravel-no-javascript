...
Route::prefix('profile')->group(function(){
    ...
    Route::get('2fa/{turn}', 'ProfileController@change2fa') -> name('profile.2fa.change'); // change 2fa
     Route::post('changetelegramusername', 'ProfileController@changeTelegram')-> name('profile.telegram.change');
    ...
    Route::get('pgp/statuschange/{status}', 'ProfileController@pgpStatusChange') -> name('profile.pgp.status');
    Route::get('pgp/show/{user_id}', 'ProfileController@showPGPKey') -> name('profile.pgp.show');
    ...
    Route::post('sale/{sale}/sent', 'VendorController@markAsSentWithMessage') -> name('profile.sales.sent.withmessage');
    ...
    Route::get('announcements','ProfileController@announcements')->name('profile.announcements');
    ...
    // Purchase - Disputes
	Route::get('purchase/{purchase}/dispute/confirmclose', 'ProfileController@confirmCloseDispute') -> name('profile.purchases.disputes.close.confirm');
	Route::get('purchase/{purchase}/closedispute', 'ProfileController@closeDispute') -> name('profile.purchases.disputes.close');
    ...
    Route::post('purchase/{purchase}/appeal/resolve', 'Admin\AdminController@resolveAppeal') -> name('profile.purchases.appeal.resolve');
    
    Route::get('purchase/{purchase}/dispute/escalate', 'DisputeController@escalateConfirm')->name('profile.dispute.escalate');
    Route::post('purchase/{purchase}/dispute/escalate', 'DisputeController@makeEscalate')->name('profile.dispute.make.escalate');
    
    // Purchase - Disputes - Appeals
    Route::get('appeal/dispute/{purchase}/confirm', 'AppealsController@make')->name('profile.purchase.dispute.appeal');
    Route::post('appeal/dispute/{purchase}/confirm', 'AppealsController@appealDisputeConfirm')->name('profile.purchase.dispute.appeal.confirm');
    ...
    //Sale sent add message
	Route::get('salemessage/decrypt/keysale','MessageController@decryptKeyShowSale')->name('profile.sales.messages.decrypt.show.sale');
	Route::post('salemessage/decrypt/keysale','MessageController@decryptKeyPostSale')->name('profile.sales.messages.decrypt.post.sale');
	...
	Route::get('ticket/{ticket}/solve', 'ProfileController@solveTicket') -> name('profile.ticket.solve');
    Route::get('report-item/{user}/{url}', 'ProfileController@ReportItemtickets') -> name('profile.tickets.reportitem');
    /**
     * Product clone
     */
     ...
     * Product Status
     */
	Route::get('product/status/{product}','ProductController@statusProductShow')->name('profile.vendor.product.status.show');
	Route::post('product/status/{product}','ProductController@statusProductPost')->name('profile.vendor.product.status.post');
